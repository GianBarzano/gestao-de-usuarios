...
class UsuarioController {
    ...
    public definirRotas(router: Router){
        router.get('/usuarios/:id', autenticacaoMiddleware, (req, res) => {
            this.buscar(req, res);
        });
    ...
    public async criar(req: Request, res: Response) {
        try {
            ...
            try {
                // Crio usuário no banco de dados
                ...
                await usuarioModel.criar(usuario);

                // Crio usuário no Firebase
                const usuarioFirebase = await firebaseModel.criarUsuario({
                    uid: usuario.id,
                    email,
                    password: senha,
                    displayName: nome
                ...
                let authToken;
                try {
                    authToken = await firebaseModel.criarCustomToken(usuarioFirebase.uid);
                } catch (error) {}
